...
This runbook covers the API in `pomodoro-node`, the web app in `PomodoroTimer`, and their MongoDB database. Complete the pre-deployment acceptance backlog before promoting a release.
...
| Database      | MongoDB                         | value is secret-managed as `MONGODB_URI`    | provider health, connections, latency, storage |
...
1. Deploy the recorded backend SHA through Render. Keep the previous successful deployment identified for rollback.
...
- Enable MongoDB provider continuous backups with point-in-time recovery when available. Target a recovery point objective of 24 hours or better and a recovery time objective of four hours or better.
...
- For a restore, select the last known-good point, restore into a new cluster, validate collection counts and representative user/timer relationships, run application smoke tests, then switch `MONGODB_URI` during an announced maintenance window. Preserve the damaged cluster read-only until the incident review permits deletion.
...
- MongoDB reports impaired health, connection usage above 80%, sustained slow queries, storage above 80%, or backup failure.
